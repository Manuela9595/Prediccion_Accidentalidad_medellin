#############################################################################################
#           MODELO DE PREDICCION MENSUAL DE ACCIDENTES - HERIDOS                            #
...
### lectura de datos #####
heridos_train <- read.csv("~/Docs/MAESTRIA/ANALITICA PREDICTIVA/Trabajo Final/ENTREGA FINAL/MENSUAL/mensual_h_train.csv", sep=",")
heridos_test <- read.csv("~/Docs/MAESTRIA/ANALITICA PREDICTIVA/Trabajo Final/ENTREGA FINAL/MENSUAL/mensual_h_test.csv", sep=";")
...
set.seed(101)
mejorRandomforest <- randomForest(HERIDOS.R ~., data = heridos_train, mtry = 3, nodesize = 11, ntree = 500, sampe_size= 0.8, importance = TRUE)
mejorRandomforest
...
write.csv(heridos_test, file="mensual_h_prediccion.csv", row.names = FALSE)
