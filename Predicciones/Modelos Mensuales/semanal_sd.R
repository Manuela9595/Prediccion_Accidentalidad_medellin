#############################################################################################
#           MODELO DE PREDICCION SEMANAL DE ACCIDENTES - SOLO DAÑOS                         #
...
### lectura de datos #####
solo_danos_train <- read.csv("~/Docs/MAESTRIA/ANALITICA PREDICTIVA/Trabajo Final/ENTREGA FINAL/SEMANAL/semanal_sd_train.csv", sep=";")
solo_danos_test <- read.csv("~/Docs/MAESTRIA/ANALITICA PREDICTIVA/Trabajo Final/ENTREGA FINAL/SEMANAL/semanal_sd_test.csv", sep=";")

solo_danos_train <- dplyr :: select(solo_danos_train, -ANYO)
solo_danos_test <- dplyr :: select(solo_danos_test, -PERIODO)


#### entrenamiento del modelo ####
set.seed(101)
mejorRandomforest <- randomForest(SOLO_DANOS ~., data = solo_danos_train, mtry = 8, nodesize = 7, ntree = 500, 
...
write.csv(solo_danos_test, file="semanal_sd_prediccion.csv", row.names = FALSE)
